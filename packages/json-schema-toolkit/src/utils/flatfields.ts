...
export function flatfields(schema: JsonSchema, prefix = ""): FieldMeta[] {
	...
	for (const [key, value] of Object.entries(properties)) {
		...
		const meta: FieldMeta = {
			path,
			type:
				(Array.isArray(value.type) ? value.type[0] : value.type) ?? "unknown",
			...
			enum: value.enum,
			uiWidget:
				((value as Record<string, unknown>).widget as string | undefined) ??
				(value.enum ? "select" : undefined),
		};

		if (value.type === "object" && value.properties) {
			meta.properties = value.properties as unknown as Record<
				string,
				FieldMeta
			>;
		}
