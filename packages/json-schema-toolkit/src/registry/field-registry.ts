...
export class FieldRegistry {
	...
	register<T extends FieldComponent>(
		...
		const formats: (string | null)[] =
			selector.format === undefined ? [null] : [selector.format];
		const widgets: (string | null)[] =
			selector.widget === undefined ? [null] : [selector.widget];
		const kinds: (FieldKind | null)[] =
			selector.kind === undefined ? [null] : [selector.kind];
