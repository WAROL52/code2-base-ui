...
function detectDraft(
  ...
  if (uri.includes("2020-12")) return "draft-2020-12";
...
function resolveRefs(
  ...
  if (schema.if) resolved.if = resolveRefs(schema.if, definitions, resolving);
  if (schema.then)
...
export function mergeAllOf(schemas: JsonSchema[]): JsonSchema {
  ...
  for (const schema of schemas) {
    ...
    if (schema.items && !merged.items) merged.items = schema.items;
